...
# Made for: TL-WR743ND v1 (https://is.gd/3NgWP7)
# Tested with OpenWrt installed on the router
# Put this in /www/cgi-bin/
#
# Examples:
...
if ! [ "$WW" -eq 1 ]; then
    ...
    # This mode is way better than sending 8 requests per frame.
    # The YY (state) variable will not be needed here.
    
    # Check if XX (led numberList) is 8 characters big, error if not
    if ! [ "${#XX}" -eq 8 ]; then
        echo "Error: Bro think he 8 💀"
        exit 0
    fi
